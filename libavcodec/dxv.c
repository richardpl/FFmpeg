                       ...
                       ptrdiff_t stride, const uint8_t *block)
{
    ...
    for (y = 0; y < 4; y++) {
        for (x = 0; x < 4; x++) {
            ...
            dst[x] = extract_component(a0, a1, a_code);
        }
        dst += stride;
    }
}
                     ...
                     const uint8_t *block)
{
    ...
    yo_subblock(plane0,      yo_indices, stride0, block);
    a_subblock(plane3,       a_indices,  stride1, block + 8);
    yo_subblock(plane0 + 4,  yo_indices, stride0, block + 16);
    a_subblock(plane3  + 4,  a_indices,  stride1, block + 24);
    yo_subblock(plane0 + 8,  yo_indices, stride0, block + 32);
    a_subblock(plane3  + 8,  a_indices,  stride1, block + 40);
    yo_subblock(plane0 + 12, yo_indices, stride0, block + 48);
    a_subblock(plane3  + 12, a_indices,  stride1, block + 56);
                               ...
                               int max_op_size0, int max_op_size1)
{
    ...
    while (dst + 10 < tex_data + tex_size) {
        ret = dxv_decompress_cgo(ctx, gb, tex_data, tex_size, op_data0, &oi0, op_size0, &dst, &state0, tab0, tab1);
        if (ret < 0)
            return ret;
        ret = dxv_decompress_cgo(ctx, gb, tex_data, tex_size, op_data1, &oi1, op_size1, &dst, &state1, tab2, tab3);
        if (ret < 0)
            return ret;
    }
                             ...
                             uint8_t *op_data, int max_op_size)
{
    ...
    int data_start = bytestream2_tell(gb);
    uint8_t *dst, *tptr0, *tptr1, *table0[256] = { 0 }, *table1[256] = { 0 };
