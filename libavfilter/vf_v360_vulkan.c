...
typedef struct V360VulkanContext {
    ...
    VulkanPipeline *pl_rot;
    VulkanPipeline *pl;
    FFVkBuffer rot_mat_buf;
    AVFrame *remap_x;
    AVFrame *remap_y;
    AVBufferRef *remap_ref;

    /* Shader updators, must be in the main filter struct */
    VkDescriptorImageInfo input_images[4];
    VkDescriptorImageInfo output_images[4];
    VkDescriptorImageInfo remap_x_image[4];
    VkDescriptorImageInfo remap_y_image[4];
    VkDescriptorBufferInfo rot_mat_desc;
...
static av_cold int init_filter(AVFilterContext *ctx, AVFrame *in)
{
    ...
    s->remap_ref = av_hwframe_ctx_alloc(ctx->hw_device_ctx);
    s->remap_x = av_frame_alloc();
    s->remap_y = av_frame_alloc();
    if (!s->remap_ref || !s->remap_x || !s->remap_y)
        return AVERROR(ENOMEM);

    hwfc = (AVHWFramesContext*)s->remap_ref->data;
    hwfc->format = AV_PIX_FMT_VULKAN,
    hwfc->sw_format = AV_PIX_FMT_GBRPF32;
    hwfc->width  = in->width;
    hwfc->height = in->height;

    err = av_hwframe_ctx_init(s->remap_ref);
    if (err < 0)
        return err;

    s->remap_x->format = hwfc->format;
    s->remap_x->width  = hwfc->width;
    s->remap_x->height = hwfc->height;

    s->remap_x->hw_frames_ctx = av_buffer_ref(s->remap_ref);
    if (!s->remap_x->hw_frames_ctx)
        return AVERROR(ENOMEM);
    av_hwframe_get_buffer(s->remap_ref, s->remap_x, 0);

    s->remap_y->format = hwfc->format;
    s->remap_y->width  = hwfc->width;
    s->remap_y->height = hwfc->height;

    s->remap_y->hw_frames_ctx = av_buffer_ref(s->remap_ref);
    if (!s->remap_y->hw_frames_ctx)
        return AVERROR(ENOMEM);
    av_hwframe_get_buffer(s->remap_ref, s->remap_y, 0);
    ...
    s->pl_rot = ff_vk_create_pipeline(ctx);
    if (!s->pl_rot)
        return AVERROR(ENOMEM);
    ...
    s->rot_opts.yaw   = s->yaw;
    s->rot_opts.pitch = s->pitch;
    s->rot_opts.roll  = s->roll;
    s->rot_opts.rot_order = s->rorder;

    { /* Create the rotation matrix shader */
        struct {
        ...
        VulkanDescriptorSetBinding desc_b = {
            ...
            .updater     = &s->rot_mat_desc,
            .buf_content = "mat4 rotation_matrix;",
        };

        err = ff_vk_create_buf(ctx, &s->rot_mat_buf,
                               sizeof(*rot_mat),
        ...
        SPIRVShader *shd = ff_vk_init_shader(ctx, s->pl_rot, "rotation_matrix",
                                             VK_SHADER_STAGE_COMPUTE_BIT);
        if (!shd)
            return AVERROR(ENOMEM);

        RET(ff_vk_add_descriptor_set(ctx, s->pl_rot, shd, &desc_b, 1, 0)); /* set 0 */
        ...
        GLSLC(1,    float pitch;                                              );
        GLSLC(1,    float roll;                                               );
        GLSLC(1,    int rot_order;                                            );
        GLSLC(0, };                                                           );
        GLSLC(0,                                                              );

        ff_vk_add_push_constant(ctx, s->pl_rot, 0, sizeof(s->rot_opts),
                                VK_SHADER_STAGE_COMPUTE_BIT);

        GLSLF(0, #define PI (%f)                                         ,M_PI);
        GLSLC(0, void main()                                                  );
        GLSLC(0, {                                                            );
        GLSLC(1,   float yaw_rad = yaw * PI / 180.f;                          );
        GLSLC(1,   float pitch_rad = pitch * PI / 180.f;                      );
        GLSLC(1,   float roll_rad = roll * PI / 180.f;                        );
        GLSLC(1,   float sin_yaw   = sin(yaw_rad);                            );
        GLSLC(1,   float cos_yaw   = cos(yaw_rad);                            );
        GLSLC(1,   float sin_pitch = sin(pitch_rad);                          );
        GLSLC(1,   float cos_pitch = cos(pitch_rad);                          );
        GLSLC(1,   float sin_roll  = sin(roll_rad);                           );
        GLSLC(1,   float cos_roll  = cos(roll_rad);                           );
        GLSLC(1,   mat4 mat_yaw;                                              );
        GLSLC(1,   mat4 mat_pitch;                                            );
        GLSLC(1,   mat4 mat_roll;                                             );
        GLSLC(1,   mat_yaw[0][0] = cos_yaw;                                   );
        GLSLC(1,   mat_yaw[0][1] = 0.f;                                       );
        GLSLC(1,   mat_yaw[0][2] = sin_yaw;                                   );
        GLSLC(1,   mat_yaw[0][3] = 0.f;                                       );
        GLSLC(1,   mat_yaw[1][0] = 0.f;                                       );
        GLSLC(1,   mat_yaw[1][1] = 1.f;                                       );
        GLSLC(1,   mat_yaw[1][2] = 0.f;                                       );
        GLSLC(1,   mat_yaw[1][3] = 0.f;                                       );
        GLSLC(1,   mat_yaw[2][0] = -sin_yaw;                                  );
        GLSLC(1,   mat_yaw[2][1] = 0.f;                                       );
        GLSLC(1,   mat_yaw[2][2] = cos_yaw;                                   );
        GLSLC(1,   mat_yaw[2][3] = 0.f;                                       );
        GLSLC(1,   mat_yaw[3][0] = 0.f;                                       );
        GLSLC(1,   mat_yaw[3][1] = 0.f;                                       );
        GLSLC(1,   mat_yaw[3][2] = 0.f;                                       );
        GLSLC(1,   mat_yaw[3][3] = 0.f;                                       );
        GLSLC(1,   mat_pitch[0][0] = 1.f;                                     );
        GLSLC(1,   mat_pitch[0][1] = 0.f;                                     );
        GLSLC(1,   mat_pitch[0][2] = 0.f;                                     );
        GLSLC(1,   mat_pitch[0][3] = 0.f;                                     );
        GLSLC(1,   mat_pitch[1][0] = 0.f;                                     );
        GLSLC(1,   mat_pitch[1][1] = cos_pitch;                               );
        GLSLC(1,   mat_pitch[1][2] = -sin_pitch;                              );
        GLSLC(1,   mat_pitch[1][3] = 0.f;                                     );
        GLSLC(1,   mat_pitch[2][0] = 0.f;                                     );
        GLSLC(1,   mat_pitch[2][1] = sin_pitch;                               );
        GLSLC(1,   mat_pitch[2][2] = cos_pitch;                               );
        GLSLC(1,   mat_pitch[2][3] = 0.f;                                     );
        GLSLC(1,   mat_pitch[3][0] = 0.f;                                     );
        GLSLC(1,   mat_pitch[3][1] = 0.f;                                     );
        GLSLC(1,   mat_pitch[3][2] = 0.f;                                     );
        GLSLC(1,   mat_pitch[3][3] = 0.f;                                     );
        GLSLC(1,   mat_roll[0][0] = cos_roll;                                 );
        GLSLC(1,   mat_roll[0][1] = -sin_roll;                                );
        GLSLC(1,   mat_roll[0][2] = 0.f;                                      );
        GLSLC(1,   mat_roll[0][3] = 0.f;                                      );
        GLSLC(1,   mat_roll[1][0] = sin_roll;                                 );
        GLSLC(1,   mat_roll[1][1] = cos_roll;                                 );
        GLSLC(1,   mat_roll[1][2] = 0.f;                                      );
        GLSLC(1,   mat_roll[1][3] = 0.f;                                      );
        GLSLC(1,   mat_roll[2][0] = 0.f;                                      );
        GLSLC(1,   mat_roll[2][1] = 0.f;                                      );
        GLSLC(1,   mat_roll[2][2] = 1.f;                                      );
        GLSLC(1,   mat_roll[2][3] = 0.f;                                      );
        GLSLC(1,   mat_roll[3][0] = 0.f;                                      );
        GLSLC(1,   mat_roll[3][1] = 0.f;                                      );
        GLSLC(1,   mat_roll[3][2] = 0.f;                                      );
        GLSLC(1,   mat_roll[3][3] = 0.f;                                      );
        GLSLC(1,   switch (rot_order) {                                       );
        GLSLC(2,   case 0:                                                    );
        GLSLC(2,       rotation_matrix = mat_yaw * mat_pitch * mat_roll;      );
        GLSLC(2,       break;                                                 );
        GLSLC(2,   case 1:                                                    );
        GLSLC(2,       rotation_matrix = mat_yaw * mat_roll * mat_pitch;      );
        GLSLC(2,       break;                                                 );
        GLSLC(2,   case 2:                                                    );
        GLSLC(2,       rotation_matrix = mat_pitch * mat_yaw * mat_roll;      );
        GLSLC(2,       break;                                                 );
        GLSLC(2,   case 3:                                                    );
        GLSLC(2,       rotation_matrix = mat_pitch * mat_roll * mat_yaw;      );
        GLSLC(2,       break;                                                 );
        GLSLC(2,   case 4:                                                    );
        GLSLC(2,       rotation_matrix = mat_roll * mat_yaw * mat_pitch;      );
        GLSLC(2,       break;                                                 );
        GLSLC(2,   case 5:                                                    );
        GLSLC(2,       rotation_matrix = mat_roll * mat_pitch * mat_yaw;      );
        GLSLC(2,       break;                                                 );
        GLSLC(1,   }                                                          );
        GLSLC(0, }                                                            );
        ...
        RET(ff_vk_init_pipeline_layout(ctx, s->pl_rot));
        RET(ff_vk_init_compute_pipeline(ctx, s->pl_rot));

        s->rot_mat_desc.buffer = s->rot_mat_buf.buf;
        s->rot_mat_desc.range  = VK_WHOLE_SIZE;

        ff_vk_update_descriptor_set(ctx, s->pl_rot, 0);
    }
    ...
    { /* Create the shader */
        ...
        VulkanDescriptorSetBinding desc_i[5] = {
            {
                .name       = "input_img",
...
static int process_frames(AVFilterContext *avctx, AVFrame *out_f, AVFrame *in_f)
{
    ...
    AVVkFrame *remap_x = (AVVkFrame *)s->remap_x->data[0];
    AVVkFrame *remap_y = (AVVkFrame *)s->remap_y->data[0];
    ...
    for (int i = 0; i < planes; i++) {
        ...
        RET(ff_vk_create_imageview(avctx, s->exec, &s->remap_x_image[i].imageView, remap_x->img[i],
                                   av_vkfmt_from_pixfmt(AV_PIX_FMT_GBRPF32)[i],
                                   ff_comp_identity_map));

        RET(ff_vk_create_imageview(avctx, s->exec, &s->remap_y_image[i].imageView, remap_y->img[i],
                                   av_vkfmt_from_pixfmt(AV_PIX_FMT_GBRPF32)[i],
                                   ff_comp_identity_map));

        s->input_images[i].imageLayout  = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        s->remap_x_image[i].imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        s->remap_y_image[i].imageLayout = VK_IMAGE_LAYOUT_GENERAL;
        s->output_images[i].imageLayout = VK_IMAGE_LAYOUT_GENERAL;
    }

    ff_vk_update_descriptor_set(avctx, s->pl_rot, 0);
    ...
    ff_vk_bind_pipeline_exec(avctx, s->exec, s->pl_rot);

    ff_vk_update_push_exec(avctx, s->exec, VK_SHADER_STAGE_COMPUTE_BIT,
                           0, sizeof(s->rot_opts), &s->rot_opts);
...
static void v360_vulkan_uninit(AVFilterContext *avctx)
{
    ...
    ff_vk_free_buf(avctx, &s->rot_mat_buf);
    av_frame_free(&s->remap_x);
    av_frame_free(&s->remap_y);
...
static const AVOption v360_vulkan_options[] = {
    ...
    {    "rorder", "rotation order",                OFFSET(rorder), AV_OPT_TYPE_INT,    {.i64=0},             0,                     5, FLAGS, "rorder"},
    {       "ypr", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=0},             0,                     0, FLAGS, "rorder"},
    {       "yrp", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=1},             0,                     0, FLAGS, "rorder"},
    {       "pyr", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=2},             0,                     0, FLAGS, "rorder"},
    {       "pry", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=3},             0,                     0, FLAGS, "rorder"},
    {       "ryp", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=4},             0,                     0, FLAGS, "rorder"},
    {       "rpy", NULL,                                         0, AV_OPT_TYPE_CONST,  {.i64=5},             0,                     0, FLAGS, "rorder"},
