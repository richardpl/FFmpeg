...
static int shuffle_horizontal(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ShufflePixelsContext *s = ctx->priv;
    ThreadData *td = arg;
    AVFrame *in = td->in;
    ...
    for (int p = 0; p < s->nb_planes; p++) {
        ...
        uint8_t *src = in->data[p] + slice_start * in->linesize[p];
        int32_t *map = s->map;
...
static int shuffle_vertical(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ShufflePixelsContext *s = ctx->priv;
    ThreadData *td = arg;
    AVFrame *in = td->in;
    ...
    for (int p = 0; p < s->nb_planes; p++) {
        ...
        for (int y = slice_start; y < slice_end; y++) {
            uint8_t *src = in->data[p] + map[y] * in->linesize[p];
...
static int shuffle_block(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ShufflePixelsContext *s = ctx->priv;
    ThreadData *td = arg;
    AVFrame *in = td->in;
    ...
    for (int p = 0; p < s->nb_planes; p++) {
        ...
        const int slice_end = (s->planeheight[p] * (jobnr+1)) / nb_jobs;
        uint8_t *dst = out->data[p] + slice_start * out->linesize[p];
        const uint8_t *src = (const uint8_t *)in->data[p];
        int32_t *map = s->map + slice_start * s->planewidth[p];
...
static int config_output(AVFilterLink *outlink)
{
    ...
    switch (s->mode) {
    case 0:
        s->shuffle_pixels = shuffle_horizontal;
        s->nb_blocks = (s->planewidth[0] + s->block_w - 1) / s->block_w;
        break;
    case 1:
        s->shuffle_pixels = shuffle_vertical;
        s->nb_blocks = (s->planeheight[0] + s->block_h - 1) / s->block_h;
        break;
    case 2:
        s->shuffle_pixels = shuffle_block;
