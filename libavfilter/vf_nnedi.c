...
typedef struct NNEDIContext {
    ...
    void (*prescreen)(AVFilterContext *ctx,
...
static const AVOption nnedi_options[] = {
    ...
    {"pscrn", "set prescreening", OFFSET(pscrn), AV_OPT_TYPE_INT, {.i64=2}, 0, 4, FLAGS, "pscrn" },
        ...
        {"new2",      NULL, 0, AV_OPT_TYPE_CONST, {.i64=3}, 0, 0, FLAGS, "pscrn" },
        {"new3",      NULL, 0, AV_OPT_TYPE_CONST, {.i64=4}, 0, 0, FLAGS, "pscrn" },
    { NULL }
};
...
static int filter_slice(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs)
{
    ...
    for (int p = 0; p < s->nb_planes; p++) {
        ...
        const int srcbuf_stride = width + 48;
        ...
        read_bytes(src_data + FFMAX(y_out - 5, 0) * src_linesize,
                   srcbuf + 24,
                   src_linesize * 2, srcbuf_stride,
                   width, 1);
        srcbuf += srcbuf_stride;

        read_bytes(src_data + FFMAX(y_out - 3, 0) * src_linesize,
                   srcbuf + 24,
                   src_linesize * 2, srcbuf_stride,
                   width, 1);
        srcbuf += srcbuf_stride;

        read_bytes(src_data + FFMAX(y_out - 1, 0) * src_linesize,
                   srcbuf + 24,
                   src_linesize * 2, srcbuf_stride,
                   width, 1);
        srcbuf += srcbuf_stride;

        in_line  = src_data + FFMAX(y_out + 1, 0) * src_linesize;
        out_line = dst_data + (y_out * dst_linesize);

        read_bytes(in_line, srcbuf + 24, src_linesize * 2, srcbuf_stride,
                   width, slice_height - last_slice);

        read_bytes(src_data + FFMIN(slice_end + 1, height - 1) * src_linesize,
                   srcbuf + 24 + srcbuf_stride * (slice_height - last_slice),
                   src_linesize * 2, srcbuf_stride,
                   width, 1);

        read_bytes(src_data + FFMIN(slice_end + 3, height - 1) * src_linesize,
                   srcbuf + 24 + srcbuf_stride * (slice_height + 1 - last_slice),
                   src_linesize * 2, srcbuf_stride,
                   width, 1);

        read_bytes(src_data + FFMIN(slice_end + 5, height - 1) * src_linesize,
                   srcbuf + 24 + srcbuf_stride * (slice_height + 2 - last_slice),
                   src_linesize * 2, srcbuf_stride,
                   width, 1);

        for (int y = 0; y < slice_end - slice_start; y += 2) {
            if (s->pscrn > 1) {
                s->prescreen(ctx, srcbuf + (y / 2) * srcbuf_stride + 24,
            ...
            if (s->prescreen > 0)
                interpolation(srcbuf + (y / 2) * srcbuf_stride + 24,
                              srcbuf_stride,
                              dstbuf + (y / 2) * dstbuf_stride,
                              prescreen_buf, width);
        }

        write_bytes(dstbuf, out_line, dstbuf_stride, dst_linesize * 2,
                    width, slice_height);
    }
...
static int config_input(AVFilterLink *inlink)
{
    ...
    s->half = ((1 << depth) - 1) / 2.f;

    switch (s->pscrn) {
    case 0:
        break;
    case 1:
        s->prescreen = process_old;
        subtract_mean_old(&s->prescreener_old, s->half);
        break;
    case 2:
    case 3:
    case 4:
        s->prescreen = process_new;
        subtract_mean_new(&s->prescreener_new[s->pscrn - 2], s->half);
        break;
    }
    ...
    s->input_size = (s->planewidth[0] + 48) * (s->planeheight[0] + 6);
